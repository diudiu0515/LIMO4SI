...
def transition_options(object_name: str, start: str, end: str, seed: str) -> tuple[list[dict[str, str]], str, str]:
    ...
    mirror = {"left-front": "right-front", "right-front": "left-front", "left": "right", "right": "left"}
    pairs = [(start, end), (end, start), (start, mirror.get(end, "right-front")), (mirror.get(start, "left-front"), end)]
    unique = []
    for pair in pairs + [(start, start), (end, end), ("left-front", "right-front"), ("right-front", "left-front")]:
        text = template.format(*pair)
        if text not in unique:
            unique.append(text)
    correct = template.format(start, end)
    alternatives = [text for text in unique if text != correct][:3]
...
def sequence_options(object_name: str, sequence: list[str], seed: str) -> tuple[list[dict[str, str]], str, str]:
    ...
    def transform(label: str, mirror: bool, invert_depth: bool) -> str:
        parts = label.split("-")
        if mirror:
            parts = [{"left": "right", "right": "left"}.get(part, part) for part in parts]
        if invert_depth:
            parts = [{"front": "behind", "behind": "front"}.get(part, part) for part in parts]
        return "-".join(parts)
    candidates = [
        [transform(value, True, False) for value in sequence],
        [transform(value, False, True) for value in sequence],
        [transform(value, True, True) for value in sequence],
    ]
    alternatives = [template.format(arrow.join(values)) for values in candidates]
    options, label = make_options(correct, alternatives, seed)
    return options, label, correct
...
def compact_state(state: dict[str, Any], object_id: str) -> dict[str, Any]:
    return {
        ...
        "forward_world": state["forward_world"], "relation": state["object_relations"][object_id],
    }
...
def build_question(spec: dict[str, Any], analysis: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    object_id = next(uid for uid, row in analysis["objects"].items() if row["instance_name"] == spec["object_name"])
    display = shown_name(spec["object_name"])
    ...
    result = {
        ...
        "object_id": object_id, "object_name": spec["object_name"],
...
def clip_path(case_id: str) -> str:
    return f"./task5_media/{case_id}.mp4"
...
def main() -> None:
    ...
    analysis = json.loads(resolve(Path(config["analysis"])).read_text(encoding="utf-8"))
    source_media = resolve(Path(config["media_source"]))
    groups = []
    for spec in config["cases"]:
        question, media = build_question(spec, analysis)
        lo, hi = (int(value) for value in spec["window_frames"])
        start_s, end_s = float(analysis["states"][lo]["time_s"]), float(analysis["states"][hi]["time_s"])
        duration = end_s - start_s
        destination = ROOT / "site/qa_benchmark/task5_media" / f"{spec['id']}.mp4"
        ensure_clip(source_media, destination, start_s, duration)
        groups.append({
            "name": spec["id"], "title": f"Task 5 · {shown_name(spec['object_name'])}",
            "video_clip": clip_path(spec["id"]),
            "original_image": f"./task5_media/{spec['id']}_gaze_evidence.jpg",
            "original_caption": "ADT RGB anchor frames · green: target 2D box · red: measured gaze projection",
            "video_window": {"source_video": str(source_media.relative_to(ROOT)), "start_sec": start_s, "duration_sec": duration},
    ...
    audit = {
        ...
        "category_counts": dict(counts), "unique_video_windows": len({(g['video_window']['start_sec'], g['video_window']['duration_sec']) for g in groups}),
        "answer_provenance": "directly computed from gaze ray / same-time 3D OBB / wearer pose annotations",
        "coordinate_policy": analysis["coordinate_frame"],
    }
